...
async function openCustomer(id) {
  ...
  try {
    ...
    const product = data.product_activity || {};
    ...
    $("#customerDrawerBody").innerHTML = `<div class="profile-hero"><div class="avatar">${escapeHtml((sub.name || sub.email).slice(0, 2).toUpperCase())}</div><div><strong>${escapeHtml(sub.name || "Unnamed customer")}</strong><div>${escapeHtml(sub.email)}</div>${statusChip(sub.status)}</div></div>
      ...
      <section class="profile-section"><h3>Product activity</h3><p>${escapeHtml(product.coverage_message || "No product data.")}</p><div class="definition-grid"><div><span>Last activity</span><strong>${escapeHtml(dateTime(product.last_activity_at))}</strong></div><div><span>Sessions</span><strong>${number(product.sessions)}</strong></div><div><span>Events</span><strong>${number(product.event_count)}</strong></div><div><span>Latest credits</span><strong>${product.latest_credit_balance === null ? "Not observed" : number(product.latest_credit_balance)}</strong></div></div><p>${(product.top_tools || []).map((item) => `<span class="tag">${escapeHtml(item.tool)} · ${number(item.count)}</span>`).join("")}</p></section>
