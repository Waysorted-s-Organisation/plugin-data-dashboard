...
async function load() {
  try {
    ...
    $("usersBody").innerHTML = data.items.map((row) => `<tr data-user-id="${row.id}" tabindex="0"><td><strong>${escapeHtml(row.name || "Unnamed user")}</strong><small>${escapeHtml(row.email)}</small></td><td>${chip(row.lifecycleStage)}</td><td>${relative(row.lastLoginAt)}<small>${escapeHtml(row.latestLoginSource || "No successful login")}</small></td><td>${number(row.completedJobs)}</td><td>${escapeHtml(row.topTool?.label || (row.pluginActiveDays ? `Active ${row.pluginActiveDays}d · no credited tool` : row.pluginActivityAvailable === false ? "Activity unavailable" : "No credited tool use"))}</td><td>${row.availableCredits === null ? '<span class="status-chip warn">Wallet missing</span>' : number(row.availableCredits)}</td><td>${escapeHtml(row.newsletter?.status || "Not linked")}</td></tr>`).join("") || '<tr><td colspan="7"><div class="empty-state">No users match these filters.</div></td></tr>';
