...
/**
 * Jobs a person completed, and where that number came from.
 *
 * Committed reservations are the billing record and stay authoritative when
 * they exist. When they do not — a run that never reached a committed
 * reservation, or a signed-out visitor who cannot hold one — the plugin's own
 * count of completed tool actions is shown instead, labelled, rather than
 * printing a 0 next to a tool the person demonstrably used. The two are never
 * added: a credited run produces both records, so a sum would count it twice.
 */
/**
 * Credits, and why there are none to show.
 *
 * A subscription granted straight from the backend writes a subscription record
 * without initialising a wallet. The row then showed only "Wallet missing",
 * which reads as "this person has nothing" when in fact they have a plan — the
 * missing wallet is the backend's gap, not theirs.
 */
function creditsCell(row) {
  if (row.availableCredits !== null) return number(row.availableCredits);
  if (row.subscriptionSource === "subscription_record") {
    return `<span class="status-chip warn">Wallet missing</span><small>${escapeHtml(row.subscriptionPlan || row.subscriptionStatus || "subscribed")} — granted without a wallet</small>`;
  }
  return '<span class="status-chip warn">Wallet missing</span>';
}

function jobsCell(row) {
  if (row.completedJobs) {
    const note = row.completedJobsSource === "observed" ? "from plugin activity"
      : row.completedJobsSource === "combined" ? `${number(row.creditedJobs)} billed, rest seen in activity`
        : "";
    return `${number(row.completedJobs)}${note ? `<small>${note}</small>` : ""}`;
  }
  if (row.pluginActivityAvailable === false) return '<span class="status-chip warn">Not measured</span>';
  return "0";
}
...
async function load() {
  try {
    ...
    $("userMetrics").innerHTML = metricCard("Users shown", number(data.pagination.total), "Matching the current filters")
      + metricCard("Activated", number(counts.activated || 0), "Completed a tool job, credited or not")
      + metricCard("Returning", number(counts.returning || 0), "Active on two or more days")
      + metricCard("Needs activation", number(counts.not_activated || 0), "No tool job of any kind recorded");
    $("usersBody").innerHTML = data.items.map((row) => `<tr ${row.anonymous ? "" : `data-user-id="${row.id}"`} tabindex="0"><td><strong>${escapeHtml(row.name || (row.anonymous ? "Signed-out visitor" : "Unnamed user"))}</strong>${row.anonymous && row.name ? '<span class="status-chip neutral">signed out</span>' : ""}<small>${row.anonymous ? escapeHtml(String(row.id).slice(0, 22)) : escapeHtml(row.email)}</small></td><td>${chip(row.lifecycleStage)}</td><td>${relative(row.lastActiveAt || row.lastLoginAt)}<small>${escapeHtml(row.lastActiveSource === "plugin" ? "plugin activity" : row.lastActiveSource === "tool job" ? "tool job" : (row.latestLoginSource || "No successful login"))}</small></td><td>${jobsCell(row)}</td><td>${row.topTool ? `${escapeHtml(row.topTool.label)}${row.topTool.credited === false ? '<small>from plugin activity</small>' : ""}` : row.pluginActivityAvailable === false ? "Activity unavailable" : "No tool use recorded"}</td><td>${row.anonymous ? "&mdash;" : creditsCell(row)}</td><td>${row.anonymous ? "&mdash;" : escapeHtml(row.newsletter?.status || "Not linked")}</td></tr>`).join("") || '<tr><td colspan="7"><div class="empty-state">No users match these filters.</div></td></tr>';
...
async function openUser(id) {
  try {
    ...
    $("drawerBody").innerHTML = `<div class="profile-hero"><strong>${escapeHtml(data.user.email)}</strong><p>${data.user.segments.map((value) => chip(value)).join(" ")}</p></div><div class="detail-grid"><div><span>Joined</span><strong>${dateTime(data.user.joinedAt)}</strong></div><div><span>Lifecycle</span><strong>${escapeHtml(data.user.lifecycleStage)}</strong></div><div><span>Tool jobs</span><strong>${number(data.user.completedJobs)}</strong><small>${data.user.completedJobsSource === "observed" ? "from plugin activity" : data.user.completedJobsSource === "combined" ? `${number(data.user.creditedJobs)} billed, rest seen in activity` : data.user.creditedJobs ? "committed reservations" : "none recorded"}</small></div><div><span>Available credits</span><strong>${data.billing ? number(data.billing.availableCredits) : "Wallet not initialized"}</strong></div><div><span>Newsletter</span><strong>${escapeHtml(data.newsletter?.subscriber?.status || "Not linked")}</strong></div><div><span>Subscription</span><strong>${escapeHtml(data.billing?.subscriptionStatus || data.user.subscription?.status || "No active subscription")}</strong>${data.user.subscription && !data.user.subscription.walletBacked ? `<small>${escapeHtml(data.user.subscription.planCode || "plan")} until ${dateTime(data.user.subscription.currentPeriodEnd)} — no wallet initialized</small>` : ""}</div><div><span>Lifetime credit spend</span><strong>${data.billing ? number(data.billing.lifetimeSpentCredits) : "Unavailable"}</strong></div></div><h3>Credited tool timeline</h3>${timeline(jobs, data.user.observedJobs ? `No credit reservations. The plugin recorded ${number(data.user.observedJobs)} completed tool job${data.user.observedJobs === 1 ? "" : "s"} for this user, which ${data.user.observedJobs === 1 ? "has" : "have"} no billing record to list here.` : "No credited tool jobs.")}<h3>Successful logins</h3>${timeline(logins, "No successful login.")}<h3>Purchases and billing</h3>${timeline(purchases, "No purchase attempts.")}<h3>Feedback and requests</h3>${timeline(voice, "No matched feedback or feature requests.")}<h3>Favorites</h3><p>${data.user.favorites.length ? data.user.favorites.map((value) => chip(value)).join(" ") : "No favorites saved."}</p><div class="notice coverage-notice"><strong>Coverage:</strong> ${escapeHtml(data.coverage.message)}</div>`;
